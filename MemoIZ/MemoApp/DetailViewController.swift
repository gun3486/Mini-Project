...
class DetailViewController: UIViewController {
    ...
    @IBAction func deleteMemo(_ sender: Any) {
        ...
        let okAtction = UIAlertAction(title: "삭제", style: .destructive){ [weak self]
            (action) in
            DataManager.shared.deleteMemo(self?.memo)
            self?.navigationController?.popViewController(animated: true)
        }
        
        alert.addAction(okAtction)
        
        let cancelAtction = UIAlertAction(title: "취소", style: .cancel, handler: nil)
        alert.addAction(cancelAtction)
